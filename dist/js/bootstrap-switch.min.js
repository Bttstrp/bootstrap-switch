...
(function(){var __slice=[].slice;(function($,window){"use strict";var BootstrapSwitch;BootstrapSwitch=(function(){BootstrapSwitch.prototype.defaults={state:true,size:'small',animate:true,disabled:false,readonly:false,onColor:'success',offColor:'danger',onText:"Enable",offText:"Disabled",labelText:"&nbsp;"};BootstrapSwitch.prototype.name="bootstrap-switch";function BootstrapSwitch(element,options){if(options==null){options={};}
this.$element=$(element);this.options=$.extend({},this.defaults,options,{state:this.$element.is(":checked"),size:this.$element.data("size"),animate:this.$element.data("animate"),disabled:this.$element.is(":disabled"),readonly:this.$element.is("[readonly]"),onColor:this.$element.data("on-color"),offColor:this.$element.data("off-color"),onText:this.$element.data("on-text"),offText:this.$element.data("off-text"),labelText:this.$element.data("label-text")});this.$on=$("<span>",{"class":""+this.name+"-handle-on "+this.name+"-"+this.options.onColor,html:this.options.onText});this.$off=$("<span>",{"class":""+this.name+"-handle-off "+this.name+"-"+this.options.offColor,html:this.options.offText});this.$label=$("<label>",{"for":this.$element.attr("id"),html:this.options.labelText});this.$wrapper=$("<div>");this.$wrapper.addClass(this.name);this.$wrapper.addClass(this.name+'-'+(this.options.state?'on':'off'));if(this.options.size!=null){this.$wrapper.addClass(this.name+'-'+this.options.size);}
if(this.options.animate){this.$wrapper.addClass(this.name+'-animate');}
if(this.options.disabled){this.$wrapper.addClass(this.name+'-disabled');}
if(this.options.readonly){this.$wrapper.addClass(this.name+'-readonly');}
if(this.$element.id){this.$wrapper.addClass(this.name+'-id-'+this.$element.id);}
this.$div=this.$element.wrap($("<div>")).parent();this.$wrapper=this.$div.wrap(this.$wrapper).parent();this.$element.before(this.$on).before(this.$label).before(this.$off);this._elementHandlers();this._handleHandlers();this._labelHandlers();this._formHandler();}
...
_this.options.state=checked;_this.$wrapper.removeClass(checked?""+_this.name+"-off":""+_this.name+"-on").addClass(checked?""+_this.name+"-on":""+_this.name+"-off");if(!skip){if(_this.$element.is(":radio")){$("[name='"+(_this.$element.attr('name'))+"']").not(_this.$element).prop("checked",false).trigger("change.bootstrapSwitch",true);}
...
_this.drag=true;if(_this.options.animate){_this.$wrapper.removeClass(""+_this.name+"-animate");}
...
_this.drag=false;_this.$element.prop("checked",parseInt(_this.$div.css("margin-left"),10)>-25).trigger("change.bootstrapSwitch");_this.$div.css("margin-left","");if(_this.options.animate){return _this.$wrapper.addClass(""+_this.name+"-animate");}};})(this),"click.bootstrapSwitch":(function(_this){return function(e){e.preventDefault();e.stopImmediatePropagation();_this.toggleState();return _this.$element.trigger("focus.bootstrapSwitch");};})(this)});};BootstrapSwitch.prototype._formHandler=function(){var $form;$form=this.$element.closest("form");if($form.data("bootstrap-switch")){return;}
return $form.on("reset.bootstrapSwitch",function(){return window.setTimeout(function(){return $form.find("input").filter(function(){return $(this).data("bootstrap-switch");}).each(function(){return $(this).bootstrapSwitch("state",false);});},1);}).data("bootstrap-switch",true);};return BootstrapSwitch;})();$.fn.extend({bootstrapSwitch:function(){var args,option,ret;option=arguments[0],args=2<=arguments.length?__slice.call(arguments,1):[];ret=this;this.each(function(){var $this,data;$this=$(this);data=$this.data("bootstrap-switch");if(!data){$this.data("bootstrap-switch",data=new BootstrapSwitch(this,option));}
